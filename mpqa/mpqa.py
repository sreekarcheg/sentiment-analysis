...
class Doc(object):
    ...
    def __init__(self, mpqa_dir, path, fname, topic, sc_path, int_path):
        ...
        teja = os.path.join(mpqa_dir, self._ann_dir, path, fname, self._ann_name)
        if not os.path.exists(teja):
            print(teja)

        # then annotations:
        with open(teja, 'r') as fo:
    ...
    def polars(self):
        ...
        def context_pol(token):
            ...
            and intensity.
            """
            logging.info('CALCULATING POLARITY')
    ...
    def features(self):
        ...
        for p in self.polars:
            ...
            # theVoc = voc
            # theP = p
            # print(voc[p.token.lower_])

            #teja5832 = voc[p.token.lower_]
            #try:
            feats['word'] = p.token.idx #voc[p.token.lower_].id
            #     print('GOOD!')
            #     print(type(p))
            #     print(type(p.token))
            #     print(type(p.token.lower_))
            # except Exception:
            #     print("BAD!")
            #     print(type(p))
            #     print(type(p.token))
            ...
            feats['before'] = before.idx if before else -1 #voc[before.lower_].id if before else -1
            feats['before_'] = before.lower_ if before else ''
            feats['after'] = after.idx if after else -1
